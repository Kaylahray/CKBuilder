...
- Hackathon: **Gone in 60ms: Fiber Network Infrastructure Hackathon** (1–15 July)
- Category: **3 — Merchant, Liquidity, LSP, and Multi-Asset Infrastructure**  
  (with Preflight Diagnostics also touching Category 1’s “can I pay?” tooling)
...
simulated billing product to a live Fiber path — first on local Docker
(Windows/WSL), then to a hosted, independently verifiable testnet deployment.
...
### Why we built this for Gone in 60ms

The brief is **infrastructure only**, not consumer apps. Explicit Category 3 example:

> *Service-metering infrastructure for pay-as-you-go products, subscriptions, API access, or micropayments.*  
> *Merchant tools for payment status / webhooks / reconciliation.*

FiberMeter sits exactly there: **merchant payment primitives + metering**, with
Preflight as payment-confidence tooling. It is meant to be reused by the next
(product) hackathon — games, AI tools, APIs — without each team reinventing billing.
...
## How we got here: local Docker

Before the hosted VPS, we proved the same loop on Windows/WSL with Docker.
Screenshots from that work are under [`./images/`](./images/).

### 1. Merchant Fiber node — keys, Docker, auth

Started from running a real Fiber testnet node (`nervos/fiber:0.9.0-rc7`) with
`ckb-cli` keys and faucet CKB.

![ckb-cli account new](./images/image.png)

![merchant fiber-node starting](./images/image-1.png)

![fiber-node in Docker Desktop](./images/image-3.png)

**Docker + RPC gotcha.** Binding RPC to `0.0.0.0:8227` so Windows/WSL host could
reach the container forced Fiber 0.9 **Biscuit auth**. We set
`rpc.biscuit_public_key` and `FIBER_RPC_TOKEN` so FiberMeter’s API can call
JSON-RPC with `Authorization: Bearer …`. Binding only `127.0.0.1` inside Docker
often can’t be port-published cleanly.

### 2. FiberMeter stack — Postgres, API, dashboard, demo

Brought up Postgres (`fiber-meter-postgres` on host **5433**), `pnpm migrate/seed`,
and `pnpm dev`:
...
![pnpm dev — API + dashboard + demo](./images/image-2.png)

Local live env essentials:
...
### 3. Fund + open_channel on the merchant node

Faucet wasn’t enough the first time (`capacity not enough` for ~500 CKB). After
funding properly:

- `connect_peer` / bootnodes → peers
- `open_channel` to public testnet node1 → negotiate → **ChannelReady**

![open_channel → NegotiatingFunding](./images/image-4.png)

![ChannelReady on merchant (8227)](./images/image-5.png)

### 4. Second node — payer (customers don’t self-pay)

Merchant node creates invoices. Paying the same invoice from the same node hits
**self-payment** rules. So we stood up **`fiber-payer`**:

- new `ckb-cli` account + key → `fiber-payer/ckb/key`
- Docker ports **8327 / 8328**
- faucet + channel until **ChannelReady**

![payer account + export](./images/image-8.png)

![payer key-info / faucet address](./images/image-9.png)

![Docker: postgres + fiber-node + fiber-payer](./images/image-10.png)

![payer list_peers + open_channel](./images/image-11.png)

![payer ChannelReady on 8327](./images/image-12.png)

### 5. Live invoice → pay → Verify → meter usage

Dashboard flow (local / manual payer):

1. Create payment request → live `fibt1…`
2. Payer `send_payment` (curl as stand-in wallet)
3. **Verify on Fiber** → status **paid**, balance funded
4. Quickstart API key → `POST /api/usage-events` → **charged**

Expired invoices fail (`invoice is expired`) — create fresh before paying.

![Payment Requests — first live pending invoice](./images/image-6.png)

![API logs creating payment request / dashboard traffic](./images/image-7.png)

![send_payment Created (after renewing expired invoice)](./images/image-13.png)

![Payment Requests — live pending invoices before/around verify](./images/image-14.png)

![Customers — Ada Demo balance after fund](./images/image-15.png)

![Preflight page — node peers/channels](./images/image-16.png)

![POST /api/usage-events → charged 12.5 CKB](./images/image-17.png)

![Usage Events — charged after live fund](./images/image-18.png)

Locally, Preflight initially talked only to the merchant `FIBER_RPC_URL`. That
worked for node health but failed as a true “can I pay?” check (self-payment).
The hosted config fixes this with `FIBER_PREFLIGHT_RPC_URL` on the payer.


---

## Hosted deployment (reference for auditors)

After the local proof, we moved the same architecture to a VPS + Netlify stack
so judges can verify without running Docker themselves.
...
| Local Docker merchant + payer path                   | Working; used for early verification         |
...
- Docker RPC on `0.0.0.0` requires biscuit auth in Fiber 0.9; native localhost RPC
  does not.
...
- Wire demo-service “Summarize” to real `recordUsage`.
...
- [x] Local Docker path proved earlier (merchant + payer + manual pay + metering)



Simulated mode remains the **zero-deps** path for evaluators who won’t hit the
hosted live stack.
...
Local Docker screenshots above document how we got there; the hosted VPS +
Netlify topology is the current reference deployment for auditors.
